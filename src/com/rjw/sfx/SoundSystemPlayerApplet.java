 ...
 * </b>
 */
public class SoundSystemPlayerApplet extends /**/JApplet/**/ /*JFrame*/
                                                             implements Runnable
{
...
//            mySoundSystem = new SoundSystem( LibraryLWJGLOpenAL.class );
            ...
            // testing preload
            mySoundSystem.loadSound("50mmTest3.ogg");
        }
        catch( SoundSystemException e )
        {
...
//                                "this computer" );
        ...
        mySoundSystem.newStreamingSource( true, "OGG Music", /*"beats.ogg"*/ /*"Epoq-Lepidoptera.ogg"*/"50mmTest3.ogg",
                                          true, 0, 0, 0,
                                          SoundSystemConfig.ATTENUATION_NONE,
                                          0 );
        mySoundSystem.newStreamingSource( true, "MIDI Music", "beethoven.mid",
                                          true, 0, 0, 0,
                                          SoundSystemConfig.ATTENUATION_NONE,
                                          0 );
        while( running )
...
/***********************************/
    ...
    private void processInput(){
    	...
    	if( keyboardInput.keyDown( KeyEvent.VK_S ) ) {
            ...
            {
                if( JavaOpenAL )
                    GameSkeleton.printDebugMessage( "JavaSound library not " +
                                        "compatible on this computer" );
                else
                    GameSkeleton.printDebugMessage( "OpenAL library not " +
                                        "compatible on this computer" );
                e.printStackTrace();
            }
            drawFrame();
    	...
    	if( keyboardInput.keyDown( KeyEvent.VK_1 ) ) {
    		mySoundSystem.quickPlay( false, "boing.wav", false, 0, 0, 0,
    				SoundSystemConfig.ATTENUATION_ROLLOFF,
    				SoundSystemConfig.getDefaultRolloff() );
    	}

    	if( keyboardInput.keyDown( KeyEvent.VK_2 ) ) {
    		mySoundSystem.quickPlay( false, "click.wav", false, 0, 0, 0,
    				SoundSystemConfig.ATTENUATION_ROLLOFF,
    				SoundSystemConfig.getDefaultRolloff() );
    	}

    	if( keyboardInput.keyDown( KeyEvent.VK_3 ) ) {
    	...
    	if( keyboardInput.keyDown( KeyEvent.VK_4 ) ) {
    	...
    	if( keyboardInput.keyDown( KeyEvent.VK_5 ) ) {
    	...
    	if( keyboardInput.keyDown( KeyEvent.VK_6 ) ) {

    		System.out.println(
    				mySoundSystem.quickPlay( false, "50mmTest3.ogg", false,
    				0, 0, 0,
    				SoundSystemConfig.ATTENUATION_ROLLOFF,
    				SoundSystemConfig.getDefaultRolloff() )
    		);
