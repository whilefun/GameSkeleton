...
/**
 * Our Speeder enemy. Faster, faster shooting, lower damage doing, easier to kill. For swarming.
 * @author rwalsh
 *
 */
public class Speeder extends Thing{
	...
	private int _hardPointOffsetY = 28;
	...
	public static final int WEAPON_TIMER = 1000;
	public static final int DYING_TIMER = 1500;
	public static final int HIT_TIMER = 1200;
	public static final int ORB_SPEED = 2;
	static final int SPEEDER_SIZE = 32;
	static final int SPEEDER_HEALTH = 50;
	 ...
	 * @param vy - y velocity
	 */
	public Speeder(int x, int y, int vx, int vy){
		
		super(ThingManager.THING_ID_SPEEDER, x, y, SPEEDER_SIZE, SPEEDER_SIZE, new Animation(), new Animation(), new Animation());
		
		_vx = vx;
		_vy = vy;
		_health = SPEEDER_HEALTH;
		...
		try{	
			_frameTile = ImageIO.read(new File(OtherStuff.SPRITE_PATH_PREFIX + OtherStuff.SPRITE_SPEEDER));
			_effectsTile = ImageIO.read(new File(OtherStuff.SPRITE_PATH_PREFIX + OtherStuff.SPRITE_EFFECTS_001));
		}catch(Exception e){
			GameSkeleton.printDebugMessage("Error loading frames for Speeder ("+e.getMessage()+")");
		}
		
		/////////////////////
		// MAIN ANIMATION //
		/////////////////////
		super.addAnimFrame(_frameTile.getSubimage(0, 0, 32, 32), 250);
		super.addAnimFrame(_frameTile.getSubimage(32, 0, 32, 32), 250);
		...
		super.addEffectAnimFrame(_effectsTile.getSubimage(0,0,SPEEDER_SIZE,SPEEDER_SIZE), 0);
		...
		super.addEffectAnimFrame(_effectsTile.getSubimage(SPEEDER_SIZE,0,SPEEDER_SIZE,SPEEDER_SIZE),250);
		super.addEffectAnimFrame(_effectsTile.getSubimage(2*SPEEDER_SIZE,0,SPEEDER_SIZE,SPEEDER_SIZE),250);
		
		// and our dying effect
		super.addEffectAnimation(new Animation());
		super.setCurrentEffectAnimation(ANIM_EFFECT_DYING);
		super.addEffectAnimFrame(_effectsTile.getSubimage(3*SPEEDER_SIZE,0,SPEEDER_SIZE,SPEEDER_SIZE),250);
		super.addEffectAnimFrame(_effectsTile.getSubimage(4*SPEEDER_SIZE,0,SPEEDER_SIZE,SPEEDER_SIZE),250);
		super.addEffectAnimFrame(_effectsTile.getSubimage(5*SPEEDER_SIZE,0,SPEEDER_SIZE,SPEEDER_SIZE),250);
		super.addEffectAnimFrame(_effectsTile.getSubimage(6*SPEEDER_SIZE,0,SPEEDER_SIZE,SPEEDER_SIZE),250);
